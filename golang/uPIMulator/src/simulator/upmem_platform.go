...
func (this *UpmemPlatform) Init(command_line_parser *misc.CommandLineParser) {
...
func (this *UpmemPlatform) IsFinished() bool {
	return this.execution == this.host.NumExecutions()
}

func (this *UpmemPlatform) Cycle() {
	this.host.Cycle()
...
func (this *UpmemPlatform) Dump() {
	...
	for _, dpu_ := range dpus {
		...
		if scheduler := dpu_.MemoryController().MemorySchedulerStatFactory(); scheduler != nil {
			lines = append(lines, scheduler.ToLines()...)
		}

		if buffer := dpu_.MemoryController().RowBufferStatFactory(); buffer != nil {
			lines = append(lines, buffer.ToLines()...)
		}
	}
